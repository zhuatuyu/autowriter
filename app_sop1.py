#!/usr/bin/env python
"""
app_sop1.py - 启动SOP1：指标体系生成与评分回写

用法：python app_sop1.py -y config/project01.yaml
"""
import asyncio
import argparse
import yaml
...
from backend.services.company_sop1 import CompanySOP1
...
class ProjectConfigLoader:
    ...
    def get_user_message(self):
        # 兼容：优先读取 SOP1 专用消息；无则回退通用 user_message
        return (
            self.project_config.get('user_message_sop1')
            or self.project_config.get('user_messages', {}).get('sop1')
...
async def main():
    parser = argparse.ArgumentParser(description='AutoWriter SOP1')
    ...
    file_paths = loader.get_file_paths().get('documents', [])
    ...
    result = await company.process_message(project_config, user_message, environment, file_paths)
    print(result)
