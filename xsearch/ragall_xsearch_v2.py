#!/usr/bin/env python3
"""
🧠 xsearch专用全局知识库管理工具 V2 - 使用自建服务
用法: 
  python ragall_xsearch_v2.py -f file1.md file2.pdf file3.txt                    # 构建向量索引
  python ragall_xsearch_v2.py -f file1.md file2.pdf file3.txt --kg              # 构建知识图谱
  python ragall_xsearch_v2.py -f file1.md file2.pdf file3.txt --kg --vector     # 同时构建
"""
...
try:
    from knowledge_graph_service import xsearch_kg
    KG_AVAILABLE = True
except ImportError as e:
    print(f"⚠️ 知识图谱服务不可用: {e}")
    KG_AVAILABLE = False
...
async def build_global_knowledge_base(file_paths: list, build_vector: bool = True, build_kg: bool = False, chunk_size: int = 512, overlap: int = 50):
    ...
    if build_kg:
        ...
        # 构建知识图谱
        if KG_AVAILABLE:
            success_kg = await xsearch_kg.build_knowledge_graph(global_kg_path)
            ...
            if success_kg:
                ...
                for query in test_queries:
                    print(f"\n🤔 查询: {query}")
                    try:
                        result = await xsearch_kg.query_knowledge_graph(query, mode="keyword", max_knowledge_sequence=3)
...
async def main():
    parser = argparse.ArgumentParser(
        description="🧠 xsearch专用全局知识库管理工具 V2 - 使用自建服务",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
🚀 使用示例:
  # 默认构建向量索引
  python ragall_xsearch_v2.py -f workspace/vector_storage/*.md
  
  # 构建知识图谱（推荐！）
  python ragall_xsearch_v2.py -f workspace/vector_storage/*.md --kg
  
  # 同时构建向量索引和知识图谱
  python ragall_xsearch_v2.py -f workspace/vector_storage/*.md --kg --vector
  
  # 仅构建知识图谱，不构建向量索引
  python ragall_xsearch_v2.py -f workspace/vector_storage/*.md --kg --no-vector
        """
    )
