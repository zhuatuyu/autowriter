...
WRITER_BASE_SYSTEM = (
    ...
    "2. 严格遵循架构师制定的写作指导和引用一致性要求（不进行外部检索）\n"
...
SECTION_WRITING_PROMPT = (
    ...
    "## 章节标题\n{section_title}\n\n"
    "## 写作要求与指导\n{instruction}\n\n"
    "## 相关事实依据（研究简报六键、网络案例摘录与简报附录“指标体系与评分摘要”）\n{factual_basis}\n\n"
    "## 📋 写作标准与质量要求\n\n"
    "### 🎯 内容要求\n"
...
SECTION_PROMPT_GENERATION_TEMPLATE = (
    ...
    "{rag_instructions}\n\n"
    "### 🔍 事实引用与一致性要求：\n"
    "1. 仅使用研究简报六键、网络案例摘录与简报附录“指标体系与评分摘要”为事实来源，不要发起任何外部检索\n"
    "2. 每个关键论点需可追溯到具体来源（简报键名/案例标题/指标名称）\n"
    "3. 如缺失信息，标注 \"信息待补充\"，避免臆测\n"
    "4. 确保表述与事实一致，避免过度延展\n"
)
...
REPORT_SECTIONS = [
    ...
    {"title": "二、综合绩效评价结论", "key": "conclusion", "prompt_template": "必须依据 research_brief.md 附录“指标体系与评分摘要”的评分结果，汇总总分与各维度分值与得分率，并引用关键 opinion 要点（无需直接读取指标表）。"},
    {"title": "三、主要成效及经验", "key": "achievements", "prompt_template": "结合数据和事实，总结成效，提炼可借鉴做法。"},
    {"title": "四、存在的问题和原因分析", "key": "problems", "prompt_template": "针对扣分项综合分析原因、依据与影响范围。"},
