...
async def test_search_engine_config():
    ...
    try:
        ...
        # 2. 直接创建阿里云搜索引擎实例，避免配置问题
        search_engine = SearchEngine(
            engine=SearchEngineType.ALIBABA,
            api_key=config.search.api_key,
            endpoint=getattr(config.search, 'endpoint', 'http://default-0t01.platform-cn-shanghai.opensearch.aliyuncs.com'),
            workspace=getattr(config.search, 'workspace', 'default'),
            service_id=getattr(config.search, 'service_id', 'ops-web-search-001')
        )
...
async def test_collect_links():
    ...
    try:
        ...
        # 测试搜索和排序URL - 修复参数名
        urls = await collect_links._search_and_rank_urls(
            topic=test_query, 
            query=test_query, 
            num_results=3,
            max_num_results=6
